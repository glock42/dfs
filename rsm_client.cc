...
rsm_protocol::status rsm_client::invoke(int proc, std::string req,
    ...
    while (1) {
        ...
        rpcc *cl = h.safebind();
        if (cl) {
            ret = cl->call(rsm_client_protocol::invoke, proc, req, rep,
                           rpcc::to(5000));
        }
        ...
        if (ret == rsm_client_protocol::NOTPRIMARY) {
            printf(
                "primary %s isn't the primary--let's get a complete list of "
                "mems\n",
                primary.c_str());
...
bool rsm_client::init_members() {
    ...
    if (cl) {
        ret =
            cl->call(rsm_client_protocol::members, 0, new_view, rpcc::to(1000));
    }
    VERIFY(pthread_mutex_lock(&rsm_client_mutex) == 0);
