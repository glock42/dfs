...
int rpcc::call1(unsigned int proc, marshall &req, unmarshall &rep, TO to) {
    ...
    int xid_rep;
    {
        ...
        if ((proc != rpc_const::bind && !bind_done_) ||
            ...
            jsl_log(JSL_DBG_1,
                    "rpcc::call1 rpcc has not been bound to dst or binding "
                    "twice\n");
    ...
    while (1) {
        if (transmit) {
            ...
            if (ch) {
                ...
                jsl_log(JSL_DBG_2,
                        "rpcc::call1 %u just sent req proc %x xid %u clt_nonce "
                        "%d\n",
...

        {
            ...
            while (!ca.done) {
                ...
                if (pthread_cond_timedwait(&ca.c, &ca.m, &nextdeadline) ==
                    ETIMEDOUT) {
    ...
    jsl_log(JSL_DBG_2,
            "rpcc::call1 %u call done for req proc %x xid %u %s:%d done? %d "
            "ret %d \n",
...
void rpcs::dispatch(djob_t *j) {
    ...
    if (h.srv_nonce != 0 && h.srv_nonce != nonce_) {
        jsl_log(JSL_DBG_2,
                "rpcs::dispatch: rpc for an old server instance %u (current "
                "%u) proc %x\n",
    ...
    if (h.clt_nonce) {
        // have i seen this client before?
        {
            ...
            if (reply_window_.find(h.clt_nonce) == reply_window_.end()) {
                ...
                jsl_log(JSL_DBG_2,
                        "rpcs::dispatch: new client %u xid %d chan %d, total "
                        "clients %d\n",
                        h.clt_nonce, h.xid, c->channo(),
                        (int)reply_window_.size());
            }
        }
        ...
        stat =
            checkduplicate_and_update(h.clt_nonce, h.xid, h.xid_rep, &b1, &sz1);
    } else {
    ...
    switch (stat) {
        case NEW:  // new request
            ...
            jsl_log(JSL_DBG_2,
                    "rpcs::dispatch: sending and saving reply of size %d for "
                    "rpc %u, proc %x ret %d, clt %u\n",
                    sz1, h.xid, proc, rh.ret, h.clt_nonce);

            if (h.clt_nonce > 0) {
                // only record replies for clients that require at-most-once
                // logic
        ...
        case DONE:  // duplicate and we still have the response
            c->send(b1, sz1);
            break;
        case FORGOTTEN:  // very old request and we don't have the response
                         // anymore
...
rpcs::rpcstate_t rpcs::checkduplicate_and_update(unsigned int clt_nonce,
    ...
    std::list<reply_t> *rem_list = &reply_window_[clt_nonce];
    std::list<reply_t>::iterator iter;
    bool found = false;
    for (iter = rem_list->begin(); iter != rem_list->end();) {
        if (iter->xid < xid_rep) {
            free(iter->buf);
            iter = rem_list->erase(iter);
            continue;
        }
        if (xid == iter->xid) {
            found = true;
    ...
    if (!found) {
        reply_t first = rem_list->front();
        if (xid < first.xid) {
            res = FORGOTTEN;
        } else {
            reply_t rem_rep = reply_t(xid);
            for (iter = rem_list->begin(); iter != rem_list->end(); iter++) {
                if (iter->xid > xid) {
                    rem_list->insert(iter, rem_rep);
                    break;
                }
            }
            if (iter == rem_list->end()) {
                rem_list->push_back(rem_rep);
            }
            res = NEW;
        }
    }
