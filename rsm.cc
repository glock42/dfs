...
void rsm::recovery() {
    ...
    while (1) {
        ...
        // If there was a commited viewchange during the synchronization,
        // restart
        // the recovery
...
bool rsm::sync_with_backups() {
    ...
    tprintf("lab7 sync_with_backups start\n");
    ...
    auto cur_view = cfg->get_view(vid_insync);
    backups.clear();
    for(auto id : cur_view) {
        if(id != primary) {
    ...
    insync = false;
    tprintf("lab7 sync_with_backups done\n");
    return true;
}
...
bool rsm::sync_with_primary() {
    ...
    tprintf("lab7 sync_with_primary start\n");
    bool sync_done = false;
    while(!sync_done && vid_insync == vid_commit) {
        sync_done = statetransfer(m);
    }
...
rsm_client_protocol::status rsm::client_invoke(int procno, std::string req,
    ...
    // You fill this in for Lab 7
    ScopedLock m(&invoke_mutex);
    VERIFY(pthread_mutex_lock(&rsm_mutex) == 0);
    tprintf("client_invoke: procno: %d, req: %s\n", procno, req.c_str());

    if (inviewchange) {
        tprintf("client_invoke: inviewchange\n");
        ret = rsm_client_protocol::BUSY;
    } else if (primary != cfg->myaddr()) {
        tprintf("client_invoke: not primary\n");
        ...
        tprintf("client_invoke: start call slave %s, primary %s\n",
                print_members(cur_view).c_str(), primary.c_str());

        for (auto host : cur_view) {
            if (host == primary) continue;

            handle h(host);
            auto cl = h.safebind();
            VERIFY(pthread_mutex_unlock(&rsm_mutex) == 0);
            ret = cl->call(rsm_protocol::invoke, procno, vs, req, res,
                           rpcc::to(1000));

            VERIFY(pthread_mutex_lock(&rsm_mutex) == 0);

            tprintf("client_invoke: call server %s finish, ret: %d\n", host.c_str(), ret);
        ...
        if (!failed) {
            ...
            tprintf("client_invoke: call primary finish, ret: %s\n", r.c_str());

        }
    }
    tprintf("client_invoke: finish\n");
    VERIFY(pthread_mutex_unlock(&rsm_mutex) == 0);
    return ret;
}
...
rsm_protocol::status rsm::invoke(int proc, viewstamp vs, std::string req,
    ...
    ScopedLock m(&rsm_mutex);
    tprintf("slave invoke: proc: %d, vs(%d, %d), req: %s\n", proc, vs.vid,
            vs.seqno, req.c_str());
    // You fill this in for Lab 7
    std::string res;

    if (vs != myvs) return rsm_protocol::ERR;

    if (inviewchange) return rsm_protocol::ERR;

    if(primary == cfg->myaddr()) return rsm_protocol::ERR;
...
/**
  * RPC handler: Inform the local node (the primary) that node m has
 * synchronized
  * for view vid
  */
rsm_protocol::status rsm::transferdonereq(std::string m, unsigned vid, int &) {
    int ret = rsm_protocol::OK;
    ScopedLock ml(&rsm_mutex);
    tprintf("lab7 transferdonereq: %s, vid %d start\n", m.c_str(), int(vid));
    // You fill this in for Lab 7
    ...
    if(!insync || vid != vid_insync) return rsm_protocol::BUSY;

    auto iter = std::find(backups.begin(), backups.end(), m);
    if(iter != backups.end()) {
        backups.erase(iter);
    }
    if(backups.size() == 0) {
...
rsm_protocol::status rsm::joinreq(std::string m, viewstamp last,
    ...
    if (cfg->ismember(m, vid_commit)) {
        ...
        // We cache vid_commit to avoid adding m to a view which already
        // contains
